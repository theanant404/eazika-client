...
import { ShopService, ShopOrder, ShopAnalytics } from '@/services/shopService';

export default function ShopDashboard() {
    const [isLoading, setIsLoading] = useState(true);
    const [analytics, setAnalytics] = useState<ShopAnalytics['metrics'] | null>(null);
        ...
        const loadDashboardData = async () => {
            try {
                ...
                const [analyticsData, ordersData] = await Promise.all([
                    ShopService.getAnalytics('Last 7 Days').catch(() => null), // Handle failure gracefully
                    ShopService.getShopOrders().catch(() => [])
                ]);
                ...
                // Set Orders (Sort by newest)
                if (ordersData && Array.isArray(ordersData)) {
                    // Sort desc by date
                    const sorted = ordersData.sort((a, b) => 
                        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
                    );
                    setRecentOrders(sorted.slice(0, 5)); // Top 5
                }

                // Determine if this is a "New Shop" (No orders yet)
                const totalOrders = parseInt(analyticsData?.metrics?.orders || '0');
                if (totalOrders === 0 && (!ordersData || ordersData.length === 0)) {
                    setIsNewShop(true);
                }
    ...
    const stats = [
        { 
            ...
            bg: isNewShop ? 'bg-gray-100 dark:bg-gray-700' : 'bg-green-100 dark:bg-green-900/30' 
        },
        { 
            title: 'Active Orders', 
            ...
            bg: isNewShop ? 'bg-gray-100 dark:bg-gray-700' : 'bg-blue-100 dark:bg-blue-900/30' 
        },
        { 
            title: 'Customers', 
            ...
            bg: isNewShop ? 'bg-gray-100 dark:bg-gray-700' : 'bg-purple-100 dark:bg-purple-900/30' 
        },
        { 
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        {isNewShop ? "Welcome to Eazika! Let's get your shop running." : "Welcome back! Here's what's happening today."}
                    </p>
                </div>
                            ...
                            <div className="flex justify-between items-start mb-2">
                                ...
                                {stat.change && (
                                    <span className={`text-[10px] font-bold flex items-center px-2 py-1 rounded-full ${
                                        isNewShop ? 'bg-gray-100 text-gray-400 dark:bg-gray-700 dark:text-gray-500' : 'bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400'
                                    }`}>
                                        {stat.change} {(!isNewShop && hasPositiveChange) && <ArrowUpRight size={10} className="ml-0.5" />}
                                    </span>
                                )}
                            </div>
                    ...
                    {recentOrders.length > 0 ? (
                        <div className="divide-y divide-gray-100 dark:divide-gray-700">
                            {recentOrders.map((order) => (
                                <div key={order.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <h4 className="font-semibold text-gray-900 dark:text-white text-sm">{order.customerName}</h4>
                                    ...
                                    <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto">
                                        ...
                                        <div className="text-right">
                                            <p className="font-bold text-gray-900 dark:text-white text-sm">₹{order.totalAmount}</p>
